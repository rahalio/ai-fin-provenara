...
Art-finance trust and fractional-ownership control plane for provenance, custody, screening, and share registers.
...
# Demo key: X-API-Key: provenara_demo_local_dev_key
```
...
```bash
docker compose up -d
TABLE_NAME=provenara-core-local AWS_ENDPOINT_URL=http://localhost:8000 node scripts/ensure-dynamo-table.mjs
```
